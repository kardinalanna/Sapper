...
class Map {
    ...
    ArrayList<Coord> around(Coord coord) { //возвраущает лист с координатами вокруг заданной ячейки
        Coord num;
    ...
    MatrixOfField getMatrix(int number){ //используется в тестах
        if (number == 1) return flagMatrix; else return bombMatrix;
    }
}
